...
#include <calibrator/models/shortrate/onefactormodels/general_hullwhite.hpp>
...
namespace HJCALIBRATOR
{
	GeneralizedHullWhite::GeneralizedHullWhite( const Handle<YieldTermStructure>& termStructure,
		...
		: GeneralizedHullWhite( termStructure->forwardRate( 0.0, 0.0, Continuous, NoFrequency ), alpha, NullParameter(), sigma, NullParameter() )
	...
	boost::shared_ptr<Lattice> GeneralizedHullWhite::tree( const TimeGrid& grid ) const
	{
													 ...
													 Time bondMaturity ) const
	{
		...
		if ( _a < std::sqrt( QL_EPSILON ) ) {
			v = sigma( maturity ) * B( maturity, bondMaturity )* std::sqrt( maturity );
		}
		else {
			v = sigma( maturity ) * B( maturity, bondMaturity )*
													 ...
													 Time bondMaturity ) const
	{
		...
		Real v;
		if ( _a < std::sqrt( QL_EPSILON ) ) {
			v = sigma( bondStart )*B( bondStart, bondMaturity )* std::sqrt( maturity );
		}
		else {
			...
			v = sigma( bondStart ) / (_a*sqrt( 2.0*_a )) * sqrt( std::max( c, 0.0 ) );
		}
		Real f = termStructure()->discount( bondMaturity );
